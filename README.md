...
> **当前状态:P0~P3 全部完成(建库 + 场景 + 仿真内核 + TCP 总机 + PySide6 界面),进入 P4 三方联调。**
...
| P4 联调 | 与 B(EMS)/ C(风机控制器)对账 + 演示彩排 | 🔄 进行中 |
...
1. 与 B(EMS)联调遥控/遥调全流程,`tools/diagnose_ems_conn.py` 排查连接问题
2. 与 C(风机控制器)对桨距闭环,确认两边 avail 口径一致
3. 演示彩排:选 2~3 个场景走一遍完整剧本(含拖拽改曲线、断线重连、暂停恢复)
