...
**完整硬件配置与接线说明见 [`docs/硬件配置与接线说明.md`](docs/硬件配置与接线说明.md)**，包含：

- 系统拓扑（PC → USB-TTL → STM32 → ESP8266 → WiFi → A 端）
- 硬件清单（BOM）与选型要求（3.3V 供电、≥500mA、2.4GHz）
- **全部引脚分配总表**（源自 `WindController.ioc`，逐条可复验）
- 逐条接线说明：电源 / ESP8266（USART1+PA4 复位）/ PC（USART2）/ SWD 调试
- 串口参数、网络地址配置、时钟树、上电烧录联调流程
- 常见故障排查对照表（AT/WiFi/TCP/REG 各错误码）

> **一句话接线**：电脑用 USB-TTL 接 PA2/PA3；ESP8266 接 PC4/PC5 + PA4(RST)；
> 全部 3.3V 共地；两边都是 115200 8N1。
...
- ✅ 通信地址可配：界面设置 IP/端口 → `set_server` 下发 → 重启回默认 + 连接后自动重推
- ✅ 参数回显展示：单片机实际生效的 5 项参数**直接显示在界面**（不只是 ✓/✗ 结论）
- ✅ 参数主动读回：`get_params` 纯查询命令，不发也能看单片机当前值；串口重连后**先读后写**
      （先显示单片机真实值，再用界面值纠正），两端失配看得见
- ✅ 链路自恢复：A 端踢连接 / WiFi 掉线 / 20s 静默 → 自动重连 + 重注册
- ✅ 串口热插拔：USB-TTL 拔出后每 2 s 自动重扫重连（换 COM 号也能认到）+ 面板「重连串口」按钮
- ✅ 硬件文档：[`docs/硬件配置与接线说明.md`](docs/硬件配置与接线说明.md)

待办（验收前）：
- ❓ A 端每 5–15 秒重启一次（sim_time 回跳到 48），曲线跟随回跳属如实反映，需查 A 端
